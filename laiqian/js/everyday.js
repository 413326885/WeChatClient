	...
	$.ajax({
		...
		success: function(data) {
			if(data.resCode == 1000){
				var html='';
				$.each(data.hisreceiptList, function(index, val) {
					html+='<li><div class="item_box"><span class="m"><strong>'+val.alternationdate+'</strong></span><span class="s green" style="color:'+val.color+';">'+val.dayincome+'</span></div></li>';
				});
				...
				currentPageNumTemp = data.hisreceiptList.length;
				if(callback) {
					callback(data);
				}
				if(flag=="start") {
