...
$(function(){
	...
	var v_traderBack=getSession("traderBack");
	...
	if(getLocal('v_stockPer')) {
		sangle = 3/2 * PI + getLocal('v_stockPer')/100 * 2 * PI;
	}
	if(getLocal('v_bondPer')) {
		eangle = 3/2 * PI - getLocal('v_bondPer')/100 * 2 * PI;
	}
	...
	var totalamt = 0;//总资产
	if(getLocal("v_totalamt")) {
		totalamt = getLocal("v_totalamt");
	}
	...
	function changePre(stockPer, bondPer, moneyPer){
		setLocal('v_stockPer', stockPer);
		setLocal('v_bondPer', bondPer);
		setLocal('v_moneyPer', moneyPer);
		...
		$.ajax({
			...
			success: function(data) {
				if(data.resCode==1000){
					...
					setLocal("v_totalamt", totalamt);
					if(v_traderBack != "true"){
						// 分割线
						oangle = 3/2 * PI;
						// 股票基金（红色）
	...
	function draw() {
		...
		if(stockPer==0 && v_traderBack == "true"){
			counterclockwiseS=false;
		}
		// 黄色为零 
		if(bondPer==0 && v_traderBack == "true"){
			clockwiseE=false;
		}
	...
	function resizeCanvas() {
		...
		if(getSettingFlag && v_traderBack == "true") {
			if(getLocal('v_stockPer') == 100){
				clockwiseS = false;
				clockwiseE = false;
			}
			if(getLocal('v_bondPer') == 100){
				counterclockwiseS = false;
				counterclockwiseE = false;
			}
			if(getLocal('v_moneyPer') == 100){
				clockwiseE = false;
				counterclockwiseS = false;
			}
		}else {
	...
	$("#js_ok_btn").on('click', function() {
		...
		$("#js_get_setting_box").hide();
		getSettingFlag = true;
		document.location.href="v_trader_ok.html";
	});
});
...
$(function() {
	...
	var stockPer = getLocal('v_stockPer');
	var moneyPer = getLocal('v_moneyPer');
	var bondPer = getLocal('v_bondPer');
	var data = [{
		name: '股票基金',
		value: stockPer,
		color: '#f94138'
	...
	$("#js_trader_ok_btn").on('click', function() {
		...
		$.ajax({
			...
			data: $.extend(publicOpts, {
				"cmd": "v_regaddremap",
				"moneyper": moneyPer/100,
				"bondper": bondPer/100,
				"stockper": stockPer/100,
			}),
			type: "POST",
			success: function(data) {
				processed();
